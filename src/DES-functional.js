(function(){
	 ...
	 *函数式编程
	 */
	function DES(message,key,unencrpty){
...
const	IP=[58,50,42,34,26,18,10,2,
	...
	//16轮加密函数
	function encrpty(messages,keys,unencrpty){
		let i=0
		if(unencrpty==='unencrpty'){
			keys.reverse()
		}
		while(i<16){
	...
	function oneEncrypt(str,key){
		...
		let Eright=EboxTrans(right)
		let midvalue=''
		let output=''
		for(let i=0;i<48;i++){
			midvalue+=Eright[i]^key[i]
		}
		let foutput=PboxTrans(SboxSearch(midvalue))
		for(let i=0;i<32;i++){
			output+=foutput[i]^left[i]
		}
		return right+output
	}
	//明文IP置换
...
DES('4c8ed9b32bac33dc','0123456789abcdef')//"c866691bf4b390ac"